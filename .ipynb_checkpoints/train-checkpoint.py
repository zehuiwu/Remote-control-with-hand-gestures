...
from utils.data_loader import VideoFolder
...
def main():
    ...
    # create model
    model = MobileNetV2(config['num_classes']).cuda()
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        buffer = (
                    torch.zeros([config['batch_size'], 3, 56, 56], device='cuda'),
                    torch.zeros([config['batch_size'], 4, 28, 28], device='cuda'),
                    torch.zeros([config['batch_size'], 4, 28, 28], device='cuda'),
                    torch.zeros([config['batch_size'], 8, 14, 14], device='cuda'),
                    torch.zeros([config['batch_size'], 8, 14, 14], device='cuda'),
                    torch.zeros([config['batch_size'], 8, 14, 14], device='cuda'),
                    torch.zeros([config['batch_size'], 12, 14, 14], device='cuda'),
                    torch.zeros([config['batch_size'], 12, 14, 14], device='cuda'),
                    torch.zeros([config['batch_size'], 20, 7, 7], device='cuda'),
                    torch.zeros([config['batch_size'], 20, 7, 7], device='cuda')
                )
        for j in range(config['clip_size']):
            img = input[:,j,:]   
            inputs = (img,) + buffer
            output = model(*inputs)
            results = torch.cat((results, output[0].unsqueeze(0)), 0)
            buffer = output[1:]  
        output = torch.mean(results,0)
